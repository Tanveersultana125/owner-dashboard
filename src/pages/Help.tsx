...
function useOwnerContext(): { user: User | null; schoolData: OwnerSchoolData | null } {
  const [user, setUser] = useState<User | null>(null);
  const [schoolData, setSchoolData] = useState<OwnerSchoolData | null>(null);
    ...
    const unsub = onSnapshot(
      ...
      (snap) => {
        ...
        setSchoolData({
          ...
          branchName: String(data.branchName || data.schoolName || ""),
          name: String(data.ownerName || data.name || user.displayName || "Owner"),
        });
      },
      (err) => console.warn("[Help] schools/{uid} listener:", err)
    );
    return unsub;
...
export default function Help() {
  const { user, schoolData } = useOwnerContext();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await createTicket({
        schoolId,
        branchId: String(schoolData?.branchId || ""),
        branchName: String(schoolData?.branchName || schoolData?.schoolName || ""),
        schoolName: String(schoolData?.schoolName || ""),
        createdBy: {
          uid: user.uid,
          email: (user.email || "").toLowerCase(),
          name: schoolData?.name || user.displayName || "Owner",
          role: "owner",
        },
        subject,
        description,
            ...
            <ul className="space-y-2">
              {filtered.map((t) => (
                <TicketRow
                  ...
                  currentUid={user?.uid || ""}
                  currentName={
                    schoolData?.name || user?.displayName || "Owner"
                  }
